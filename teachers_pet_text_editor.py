...
class SimpleTextEditor():
    ...
    def __init__(self):
        ...
        # scale hour(s)
        self.scale_hr = Scale(self.frm_scale_hr, from_ = 0, to = 4, 
                                  orient = 'horizontal', length = 85, command = self.verify_scale_hr)
        ...
        # button stop
        self.btn_stop = Button(self.frm_timer, text = 'stop', style = 'TButton',
        ...
        self.timer_label = Label(self.frm_timer, font = ('BOLD', 14,), anchor =('center'), width = 30,
        ...
        if not file:
            ...
            try:
                with open(file) as input:
                    ...
                    #global original (debug purpose)
                    #original = self.text (debug purpose)
                self.edit_text.delete('1.0', END)
                self.edit_text.insert(END, self.text,)
                #global editor (debug purpose)
                #editor = self.edit_text.get('1.0', tk.END) (debug purpose)
    ...
    def close_window(self):
        ...
        if self.root.title() == 'Teacher\'s Pet':
            ...
            if current_text != '':
                ...
                self.root.destroy()
        # global trimmed_current_text (debug purpose)
        # trimmed_current_text = current_text (debug purpose)
